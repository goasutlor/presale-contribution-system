...
aws iam create-role --role-name AppRunnerServiceRole --assume-role-policy-document file://trust-policy.json --region $Region 2>$null
if ($LASTEXITCODE -ne 0) {
...
aws iam attach-role-policy --role-name AppRunnerServiceRole --policy-arn "arn:aws:iam::aws:policy/service-role/AppRunnerServicePolicyForECRAccess" --region $Region 2>$null
if ($LASTEXITCODE -ne 0) {
